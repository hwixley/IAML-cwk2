...
\\
        We can deduce this is due to the fact that PCA is unsupervised so it does nto know anything about the class labels of our data. This ultimately means PCA can pick directions that make it hard to seperate classes. Furthermore, PCA should mainly be used for variables which are strongly correlated as this correlation means that when reducing the dimensions of our data it will hold more detail.
      \end{answerbox}
   ...
   \begin{subquestion}{(4 points)
      ...
      \begin{answerbox}{50em}
        ...
        These regions are far better than the ones we found in Q2.3 this is evident by the far more even class distribution. What is particularly interesting is how similar these regions are in comparison to collection of classes in the 2D-PCA plane of our training dataset in Q1.8.\\
\\
        These regions are in the same structure as the ones found in Q2.3 however each has much more significant size regions. We can deduce this difference is due to the fact that our data is not linearly seperable and thus a SVM model with a nonlinear kernel (RBF) can seperate the data (and thus classify) more effectively than a logistic regression model.
      \end{answerbox}
...
\\
   ...
   \begin{subquestion}{(6 points)
      ...
      \begin{answerbox}{52.5em}
        \scriptsize{
        \begin{center}
        \includegraphics[width=0.85\textwidth]{images/q35.png}
        \end{center}
        \begin{center}
